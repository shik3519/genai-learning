...
**Secondary — SOTA / multi-vector + late-interaction retrieval (the topic you flagged from P1 — full treatment here):** Start with [Clavié — P4: Late Interaction Models For RAG](https://hamel.dev/notes/llm/rag/p4_late_interaction.html) as the accessible entry point, then go deeper with the ColBERT paper. Multi-vector retrieval as the general idea (multiple vectors per doc/query instead of one) — ColBERT's late-interaction/max-sim scoring is the prominent instance; where it sits between dual- and cross-encoders on the accuracy/speed curve, and at what storage cost. SPLADE — learned sparse retrieval. Read enough to compare tradeoffs across bi-encoder / multi-vector-late-interaction / cross-encoder / hybrid for a given latency budget — this is an interview favorite and directly relevant to your Amazon project. **Multimodal retrieval (landscape-level, the other topic you flagged):** CLIP-style dual encoders across text/image — same dual-encoder contrastive-training idea from week 2, just cross-modal. This stays conceptual-only; hands-on multimodal implementation is still out of scope per the plan's cuts.
**Reading:** [ColBERT paper](https://arxiv.org/abs/2004.12832) intro + architecture; [SPLADE paper](https://arxiv.org/abs/2107.05720) abstract + section 2; [CLIP paper](https://arxiv.org/abs/2103.00020) abstract + architecture (for multimodal retrieval — skim, not implement)
...
**Reading:** [RAGAS paper](https://arxiv.org/abs/2309.15217) sections 1–3; [Clavié — P2: Modern IR Evals For RAG](https://hamel.dev/notes/llm/rag/p2-evals.html) — its point on evaluating coverage/diversity rather than just RAGAS-style single-answer metrics is directly relevant to this week's eval harness. If you want the bridge into Phase 2's RL work and the paper's likely angle, skim [Clavié — P3: Optimizing Retrieval with Reasoning Models](https://hamel.dev/notes/llm/rag/p3_reasoning.html) too — this is the practitioner-side version of the Rank-K paper already flagged in the research-paper track.
