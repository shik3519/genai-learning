...
- **Practitioner sentiment matches the venue trend:** Hamel Husain's ["Stop Saying RAG Is Dead"](https://hamel.dev/notes/llm/rag/not_dead.html) (industry, not a venue paper, but widely read) makes the same case from the applied side — reasoning-enabled retrievers and late-interaction over bigger context windows, plus evaluation that goes beyond single-answer IR metrics. Worth reading during the survey milestone as the practitioner-side complement to the NeurIPS/SIGIR academic trend above; its eval critique (coverage/diversity, not just RAGAS-style metrics) is also directly relevant to whatever evaluation section the paper ends up needing.
